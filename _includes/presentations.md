## {% if page.lang == "ja" %}セミナー{% else %}Seminars{% endif %}

- "測定の間主観性による物理量と非古典性の操作的特徴付け"  
  [25th PIS Lab Seminar](https://sites.google.com/view/pis-univ-tsukuba/seminar-visitors),
  {% if page.lang == "ja" %} 筑波大学, 2026年4月
  {% else %} University of Tsukuba, Apr. 2026
  {% endif %}

## {% if page.lang == "ja" %}口頭発表{% else %}Oral Presentations{% endif %}

- "測定とエントロピーから見た一般確率論と熱力学第二法則の整合性"  
  [第54回量子情報技術研究会 (QIT54)](https://www.ieice.org/es/qit/qit54/index.html),
  {% if page.lang == "ja" %} 三重県伊勢市, 2026年5月
  {% else %} Mie, Japan, May 2026
  {% endif %}

- "一般確率論における測定の間主観性による物理量の特徴付け"  
  [Quantum Foundations 2026](https://www.math.is.tohoku.ac.jp/~quantum/QF26/index.html),
  {% if page.lang == "ja" %} 大阪大学, 2026年3月
  {% else %} The University of Osaka, Japan, Mar. 2026
  {% endif %}

- "量子通信路の純粋化"  
  [第70回物性若手夏の学校](https://cmpss.jp/ss2025/),
  {% if page.lang == "ja" %} 滋賀県高島市, 2025年9月
  {% else %} Shiga, Japan, Sep. 2025
  {% endif %}

## {% if page.lang == "ja" %}ポスター{% else %}Posters{% endif %}

- "Sharpness and intersubjectivety of measurements in generalized probabilistic theories"  
  [Quantum Resources 2026](https://2026.quantumresources.science/),
  {% if page.lang == "ja" %} 東京都中央区, 2026年5月
  {% else %} Tokyo, Japan, Mar. 2026
  {% endif %}

- "Theoretical study on quantum channel purification"  
  [量子力学100周年研究会：量子基礎・量子情報のこれまでとこれから](https://indico.yukawa.kyoto-u.ac.jp/event/64/),
  {% if page.lang == "ja" %} 京都大学, 2025年9月
  {% else %} Kyoto University, Japan, Sep. 2025
  {% endif %}

- "量子通信路の純粋化"  
  [第70回物性若手夏の学校](https://cmpss.jp/ss2025/),
  {% if page.lang == "ja" %} 滋賀県高島市, 2025年9月
  {% else %} Shiga, Japan, Sep. 2025
  {% endif %}
